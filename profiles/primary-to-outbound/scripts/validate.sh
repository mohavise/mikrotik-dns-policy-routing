...
for service in apple-app-store canva debian docker facebook figma github google-drive google-play instagram linkedin openai proxmox redhat signal spotify steam telegram ubuntu whatsapp x youtube; do
    grep -q "$service:" "$output_file"
done

if [ "$(grep -c 'type=FWD' "$output_file")" -lt 172 ]; then
