...
while IFS= read -r service_conf; do
    ...
    database_dir="$service_root/database"
    . "$service_conf"
    ...
    scheduler="update-$service_id-outbound"
...
while IFS= read -r services_file; do
    ...
    list_name="DST-$(printf '%s' "$category_id" | tr '[:lower:]-' '[:upper:]_')-TO-OUTBOUND"
    list_name="$(printf '%s' "$list_name" | tr '_' '-')"
    ...
    scheduler="update-$category_id-to-outbound"
...
| [$SERVICE_NAME](./$service_id/) | \`$LIST_NAME\` |"
    ...
    cat > "$category_root/README.md" <<EOF
...
printf 'Generated installation README files for all categories and services\n'
