...
grep -q 'google-drive' "$output_dir/list-all.rsc"
grep -q 'DST-GOOGLE-DRIVE-TO-OUTBOUND' "$output_dir/list-all.rsc"

if [ "$(grep -c 'type=FWD' "$output_dir/list-domains.rsc")" -lt 8 ]; then
    echo "Too few Google Drive domain entries"
    exit 1
fi
