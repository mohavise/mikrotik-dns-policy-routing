...
grep -q 'apple-app-store' "$output_dir/list-all.rsc"
grep -q 'DST-APPLE-APP-STORE-TO-OUTBOUND' "$output_dir/list-all.rsc"

if [ "$(grep -c 'type=FWD' "$output_dir/list-domains.rsc")" -lt 13 ]; then
    echo "Too few Apple App Store domain entries"
    exit 1
fi
