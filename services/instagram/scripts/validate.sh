...
grep -q 'instagram' "$output_dir/list-all.rsc"
grep -q 'DST-INSTAGRAM-TO-OUTBOUND' "$output_dir/list-all.rsc"

if [ "$(grep -c 'type=FWD' "$output_dir/list-domains.rsc")" -lt 2 ]; then
    echo "Too few Instagram domain entries"
    exit 1
fi
