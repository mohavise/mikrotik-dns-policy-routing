...
grep -q 'redhat' "$output_dir/list-all.rsc"
grep -q 'DST-REDHAT-TO-OUTBOUND' "$output_dir/list-all.rsc"

if [ "$(grep -c 'type=FWD' "$output_dir/list-domains.rsc")" -lt 5 ]; then
    echo "Too few Red Hat domain entries"
    exit 1
fi
