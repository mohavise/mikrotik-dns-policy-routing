...
:local url \"https://raw.githubusercontent.com/mohavise/mikrotik-dns-policy-routing/main/services/docker/output/list-all.rsc\"
...
:do { /tool fetch url=\$url dst-path=\$fileName mode=https } on-error={ :log warning \"Docker outbound update: download failed; keeping old list\"; :return }
...
:if (\$fileSize < \$minFileSize) do={ :log warning (\"Docker outbound update: downloaded file too small (\" . \$fileSize . \" bytes); keeping old list\"); /file remove \$fileName; :return }
