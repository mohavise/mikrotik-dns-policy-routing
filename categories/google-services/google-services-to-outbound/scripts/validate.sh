...
grep -q 'DST-GOOGLE-SERVICES-TO-OUTBOUND' "$output_file"

for service in google-drive youtube; do
    grep -q "$service:" "$output_file"
done

if [ "$(grep -c 'type=FWD' "$output_file")" -lt 18 ]; then
    echo "Too few Google Services domain entries"
    exit 1
fi
