...
grep -q 'openai' "$output_dir/list-all.rsc"
grep -q 'DST-OPENAI-TO-OUTBOUND' "$output_dir/list-all.rsc"

if [ "$(grep -c 'type=FWD' "$output_dir/list-domains.rsc")" -lt 12 ]; then
    echo "Too few OpenAI domain entries"
    exit 1
fi
