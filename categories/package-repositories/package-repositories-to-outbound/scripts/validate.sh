...
grep -q 'DST-PACKAGE-REPOSITORIES-TO-OUTBOUND' "$output_file"

for service in debian docker proxmox redhat ubuntu; do
    grep -q "$service:" "$output_file"
done

if [ "$(grep -c 'type=FWD' "$output_file")" -lt 30 ]; then
    echo "Too few package repository domain entries"
    exit 1
fi
