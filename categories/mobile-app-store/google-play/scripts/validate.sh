...
grep -q 'google-play' "$output_dir/list-all.rsc"
grep -q 'DST-GOOGLE-PLAY-TO-OUTBOUND' "$output_dir/list-all.rsc"

if [ "$(grep -c 'type=FWD' "$output_dir/list-domains.rsc")" -lt 35 ]; then
    echo "Too few Google Play domain entries"
    exit 1
fi
