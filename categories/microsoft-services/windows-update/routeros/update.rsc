...
add dont-require-permissions=no name=update-windows-update-outbound owner=admin policy=ftp,read,write,policy,test source=":local fileName \"windows-update-outbound.rsc\"
...
:if (\$payloadSize = 0) do={ /file remove [find name=\$fileName]; :log warning \"Windows Update outbound update: downloaded file failed validation; keeping old list\"; :return \"\" }
...
:do { /import file-name=\$fileName } on-error={ :log error \"Windows Update outbound update: import failed; downloaded file removed\"; /file remove [find name=\$fileName]; :return \"\" }
:if (([:len [/ip dns static find address-list=\$addrList]] = 0) && ([:len [/ip firewall address-list find list=\$addrList]] = 0)) do={ :log error \"Windows Update outbound update: imported list is empty; downloaded file removed\"; /file remove [find name=\$fileName]; :return \"\" }
/file remove [find name=\$fileName]
:log warning \"Windows Update outbound update: completed successfully; downloaded file removed\""
