...
:do { /tool fetch url=\$url dst-path=\$fileName mode=https } on-error={ :log warning \"OneDrive outbound update: download failed; keeping old list\"; :return }
...
:if (\$fileSize < \$minFileSize) do={ :log warning (\"OneDrive outbound update: downloaded file too small (\" . \$fileSize . \" bytes); keeping old list\"); /file remove \$fileName; :return }
